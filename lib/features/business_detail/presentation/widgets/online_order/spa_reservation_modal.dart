...
class _SpaReservationModalState extends State<SpaReservationModal> {
  ...
  // Page 2 : Informations personnelles, date, heure, thérapeute
  Widget _buildInformationPage(bool isSmallScreen, double horizontalPadding) {
    final therapists = widget.business.specificData['therapists'] as List? ?? [];
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // Sélection du thérapeute (optionnel)
          if (therapists.isNotEmpty) ...[
            ...
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: therapists.map<Widget>((therapist) {
                final name = therapist['name'] ?? '';
                ...
                return ChoiceChip(
                  ...
                  onSelected: (selected) {
                    setState(() {
                      _data.selectedTherapist = selected ? name : null;
                    });
                  },
                    ...
                    child: Column(
                      children: [
                        ...
                        _buildSummaryRow(
                          "Date",
                          _data.appointmentDate != null
                              ? "${_data.appointmentDate!.day}/${_data.appointmentDate!.month}/${_data.appointmentDate!.year}"
                              : "Non renseignée",
                          isSmallScreen,
                        ),
                        _buildSummaryRow(
                          "Heure",
                          _data.appointmentTime != null
                              ? _data.appointmentTime!.format(context)
                              : "Non renseignée",
                          isSmallScreen,
                        ),
                        if (_data.selectedTherapist != null)
                          _buildSummaryRow(
                            "Thérapeute",
                            _data.selectedTherapist!,
                            isSmallScreen,
                          ),
